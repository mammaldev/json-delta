...
function mergeConflicts ( theirsDiff, yoursDiff ) {
  ...
  while (
    ...
    /*
    console.log('tVars.currChange:', tVars.currChange);
    console.log('yVars.currChange:', yVars.currChange);
    console.log('tVars.ix: %d, yVars.ix: %d, order: %s', tVars.ix, yVars.ix, order);
    //*/
    ...
    if (
      ...
      //    -> array itslef not deleted / updated on either theirs or yours
...
function orderChanges ( c1, c2 ) {
  for (
      var k1, k2, i = 0;
      k1 = c1.path[ i ], k2 = c2.path[ i ], ( k1 !== void(0) || k2 !== void(0) );
      i++
  ) {
    if ( k1 !== k2 ) {
      return ( typeof k1 === 'undefined' || k1 < k2 ) ? -1 : 1;
    }
  }
  // same path
  return Number(isArrayItemsChange(c1)) - Number(isArrayItemsChange(c2)); // non-array-item changes first
}
